...
# 내부 유틸
# =========================================================
def move_servo_slow(servo_obj, start, end, step=2, delay=0.03):
    """
    서보를 start → end까지 천천히 이동
    """
    ...
    try:
        ...
        for idx, pos in enumerate(PATH):
            ...
            if idx > 0:
                ...
                # ✅ 핵심 수정:
                # (2,0)->(2,1)은 "반원(니 TURN_SEC_LEFT로 튜닝)" 한 번 돌고
                # 그 다음 "직선 30cm"만 가게 만들고
                # 4조 도착 후 추가 회전 안 하게 heading을 서쪽(2)로 고정
                if (x0, y0) == (2, 0) and (x1, y1) == (2, 1):
                    print("[MOVE] (2,0)->(2,1): arc then straight 1 cell, no extra turn at group4")
                    turn_left_90(motors, steer_srv)   # 니가 반원으로 맞춘 값 그대로
                    forward_cells(motors, 1)          # 직선 30cm
                    global heading
                    heading = 2                       # 4조 도착 시점에 서쪽 바라보게 고정
                else:
                    dx, dy = x1 - x0, y1 - y0
